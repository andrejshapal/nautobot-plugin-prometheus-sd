...
def _is_compose_included(context, name):
    return f"docker-compose.{name}.yml" in context.nautobot_prometheus_sd.compose_files
...
def docker_compose(context, command, **kwargs):
    ...
    compose_command_tokens = [
        "docker compose",
        f"--project-name {context.nautobot_prometheus_sd.project_name}",
        f'--project-directory "{context.nautobot_prometheus_sd.compose_dir}"',
    ]
...
def _ensure_creds_env_file(context):
    ...
    if not os.path.exists(os.path.join(context.nautobot_prometheus_sd.compose_dir, "creds.env")):
        ...
        shutil.copy(
            os.path.join(context.nautobot_prometheus_sd.compose_dir, "creds.example.env"),
            os.path.join(context.nautobot_prometheus_sd.compose_dir, "creds.env"),
        )
...
def _get_docker_nautobot_version(context, nautobot_ver=None, python_ver=None):
    ...
    if python_ver is None:
        python_ver = context.nautobot_prometheus_sd.python_ver
    dockerfile_path = os.path.join(context.nautobot_prometheus_sd.compose_dir, "Dockerfile")
...
def lock(context, check=False, constrain_nautobot_ver=False, constrain_python_ver=False):
    ...
    if constrain_nautobot_ver:
        ...
        if constrain_python_ver:
            command += f" --python {context.nautobot_prometheus_sd.python_ver}"
        try:
            ...
            command = f"poetry add --lock git+https://github.com/nautobot/nautobot.git#{context.nautobot_prometheus_sd.nautobot_ver}"
            if constrain_python_ver:
                command += f" --python {context.nautobot_prometheus_sd.python_ver}"
            run_command(context, command)
    else:
...
def pylint(context):
    ...
    command = f"{base_pylint_command} nautobot_prometheus_sd"
    if not run_command(context, command, warn=True):
        exit_code = 1

    # run the pylint_django migrations checkers on the migrations directory, if one exists
    migrations_dir = Path(__file__).absolute().parent / Path("nautobot_prometheus_sd") / Path("migrations")
    if migrations_dir.is_dir():
        migrations_pylint_command = (
            ...
            " nautobot_prometheus_sd.migrations"
        )
...
def tests(context, failfast=False, keepdb=False, lint_only=False):
    ...
    if not is_truthy(context.nautobot_prometheus_sd.local):
...
def validate_app_config(context):
    ...
    start(context, service="nautobot")
    nbshell(
        context, plain=False, file="development/app_config_schema.py", env={"APP_CONFIG_SCHEMA_COMMAND": "validate"}
    )
